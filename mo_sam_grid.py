...
def main(comm, inv_n, mask_n, current_time_str):
    P_INV = inv_n
    TINY = 1e-10
    COMM = comm
    FEA = "DTS"
    ...
    poly_root = "./ANNO/" + COMM
    if mask_n >= 0:
        SAVE_STR = "_P_interval" + str(P_INV) + "_MN" + str(mask_n)
    else:
        SAVE_STR = "_P_interval" + str(P_INV)
    TEMP_STR = "./Results/" + current_time_str + "_Grid/"
    ...
    def generate_PN_points(image, polygon, pixel_interval):
        if pixel_interval > 0:
            ...
            hh, ww = image.shape[0], image.shape[1]
            pos_points = [find_center_in_polygon(polygon, hh, ww)]
            return pos_points, []
    ...
    def inference_one_img(img_path, point_path, ip, pp):
        ...
        for pl in poly_list:
            # point prompts
            pos_points, neg_points = generate_PN_points(image, np.array(pl, np.int32), pixel_interval=P_INV)
            pos_labels = []
...
for c in args.comm_list:
    for iv in args.iv_list:
        print('start', c, iv)
        main(comm=c, inv_n=iv, mask_n=-1, current_time_str=current_time_str)
        print('one job done!')
