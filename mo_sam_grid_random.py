...
def main(comm, rn_n, inv_n, mask_n, current_time_str):
    P_INV = inv_n
    RN_N = rn_n
    TINY = 1e-10
    COMM = comm
    ...
    poly_root = "./ANNO/" + COMM
    if mask_n >= 0:
        SAVE_STR = "_P_interval" + str(P_INV) + "_RAN" + str(RN_N) + "_MN" + str(mask_n)
    else:
        SAVE_STR = "_P_interval" + str(P_INV) + "_RAN" + str(RN_N)
    TEMP_STR = "./Results/" + current_time_str + "_GridRandom/"
    ...
    def generate_PN_points(image, polygon, pixel_interval, rn):
        if pixel_interval > 0:
            x, y, w, h = cv2.boundingRect(polygon)
            all_xs = []
            curr_x = x
            while curr_x <= w + x:
                all_xs.append(curr_x)
                curr_x += pixel_interval
            all_xs.append(x + w)
            all_ys = []
            curr_y = y
            while curr_y <= h + y:
                all_ys.append(curr_y)
                curr_y += pixel_interval
            all_ys.append(y + h)

            pos_points, neg_points = [], []
            for ax in all_xs:
                for ay in all_ys:
                    if cv2.pointPolygonTest(polygon, (ax, ay), False) > 0:
                        pos_points.append((ax, ay))
                    else:
                        neg_points.append((ax, ay))
            if len(pos_points) == 0:
                hh, ww = image.shape[0], image.shape[1]
                pos_points.append(find_center_in_polygon(polygon, hh, ww))
            pos_points_f = random.sample(pos_points, rn) if rn < len(pos_points) else pos_points
            neg_points_f = random.sample(neg_points, rn) if rn < len(neg_points) else neg_points
            return pos_points_f, neg_points_f
        else:
    ...
    def inference_one_img(img_path, point_path, ip, pp):
        ...
        for pl in poly_list:
            # point prompts
            pos_points, neg_points = generate_PN_points(image, np.array(pl, np.int32), pixel_interval=P_INV, rn=RN_N)
            pos_labels = []
            ...
            if DEBUG_P:
                ...
                if len(neg_points) > 0:
                    masks, scores, logits = predictor.predict(
                        ...
                        multimask_output=mask_n >= 0,
                    )
                else:
                    assert P_INV <= 0
                    masks, scores, logits = predictor.predict(
                        point_coords=np.array(pos_points),
                        point_labels=np.array(pos_labels),
                        multimask_output=mask_n >= 0,
                    )
                # show results
...
parser.add_argument('--rn_list', type=int, nargs='+',
                    help="The number of randomly selected positive and negative point prompt from Grid.")
parser.add_argument('--iv_list', type=int, nargs='+', help="The number of pixel interval for point prompt.")
args = parser.parse_args()

for c in args.comm_list:
    for rn in args.rn_list:
        for iv in args.iv_list:
            print('start', c, rn, iv)
            main(comm=c, rn_n=rn, inv_n=iv, mask_n=-1, current_time_str=current_time_str)
            print('one job done!')
